...
try:
    ...
    try:
        from conversor_model import BaseModel
    except ImportError:
...
try:
    ...
    try:
        from db_adapter import upsert_cancion, registrar_descarga  # type: ignore
        _DB_ADAPTER_OK = True
    except Exception:
...
class SoundCloudConverter(BaseModel):
    """Conversor robusto de SoundCloud a MP3"""
    ORIGIN_SOUNDCLOUD = "SoundCloud"

    def __init__(self, download_folder=None):
        super().__init__(self.ORIGIN_SOUNDCLOUD)
        # Usar carpeta por defecto de BaseModel si no se especifica
        folder = download_folder if download_folder is not None else self.download_folder
        self.download_folder = Path(folder)
        self.download_folder.mkdir(parents=True, exist_ok=True)

    def set_download_folder(self, path):
        """
        Cambia la carpeta de descarga (sobrescribe para mantener tipo Path).
        """
        super().set_download_folder(path)
        # Convertir a Path para que las operaciones con / sigan funcionando
    ...
    def get_track_info(self, url):
        """Extrae información del track de SoundCloud en formato canónico estandarizado"""
        try:
            ...
            if result.returncode == 0:
                ...
                genres_raw = info.get('genres') or info.get('tags') or []
                genero = ', '.join(genres_raw[:3]) if genres_raw else 'Género Desconocido'
                return {
                    # ── Claves canónicas (mismo formato que spotify/youtube) ──
                    'titulo':           info.get('title', 'Título Desconocido'),
                    'artista':          info.get('uploader', info.get('artist', 'Artista Desconocido')),
                    'album':            info.get('album', 'SoundCloud'),
                    'duracion_seg':     int(info.get('duration') or 0),
                    'genero':           genero,
                    'plataforma_origen': 'SoundCloud',
                    'url_origen':       url,
                    'ruta_local':       '',  # Se llenará tras la descarga
                    'caratula_url':     info.get('thumbnail') or '',
        ...
        titulo_fb = parts[-1].replace('-', ' ').title() if len(parts) >= 1 else 'Título Desconocido'
        artista_fb = parts[-2].replace('-', ' ').title() if len(parts) >= 2 else 'Artista Desconocido'
        return {
            'titulo':           titulo_fb,
            'artista':          artista_fb,
            'album':            'SoundCloud',
    ...
    def _enhance_metadata_mutagen(self, file_path, track_info):
        ...
        try:
            ...
            # Metadatos básicos (claves canónicas con fallback a claves antiguas)
            titulo  = track_info.get('titulo',  track_info.get('title', 'Título Desconocido'))
            artista = track_info.get('artista', track_info.get('artist', 'Artista Desconocido'))
            album   = track_info.get('album', 'SoundCloud')
            audio.tags.add(TIT2(encoding=3, text=titulo))
            audio.tags.add(TPE1(encoding=3, text=artista))
            audio.tags.add(TALB(encoding=3, text=album))
            audio.tags.add(TPE2(encoding=3, text=artista))
            audio.tags.add(COMM(encoding=3, lang='eng', desc='Description', 
                                text=track_info.get('descripcion', track_info.get('description', ''))[:100]))
            
            # Descargar y agregar thumbnail si está disponible
            caratula_url = track_info.get('caratula_url', track_info.get('thumbnail', ''))
            if caratula_url:
                try:
                    response = requests.get(caratula_url, timeout=10)
    ...
    def _enhance_metadata_eyed3(self, file_path, track_info):
        ...
        try:
            ...
            audiofile.tag.album_artist = artista
    ...
    def convert(self, url, custom_name=None):
        ...
        if _DB_ADAPTER_OK:
            try:
                metadata_bd = {
                    'titulo':            track_info['titulo'],
                    'artista':           track_info['artista'],
                    'album':             track_info.get('album', 'SoundCloud'),
                    'duracion_seg':      track_info.get('duracion_seg') or None,
                    'genero':            track_info.get('genero') or None,
                    'plataforma_origen': 'SoundCloud',
                    'url_origen':        url,
                    'ruta_local':        mp3_abs,
                    'caratula_url':      track_info.get('caratula_url') or None,
                }
