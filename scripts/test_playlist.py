...
print("\n3⃣ Probando ver detalles de playlist...")
if playlists:
    id_test = playlists[0]['id_playlist']
    success, msg, playlist = controller.get_playlist_details(id_test)
    if success:
        print(f"   [OK] Playlist: {playlist['nombre']}")
        print(f"   [OK] Canciones: {playlist['total_canciones']}")

# Test 4: Buscar canciones
print("\n4⃣ Probando búsqueda de canciones...")
success, msg, resultados = controller.search_songs_by_query("Canción")
print(f"   {msg}")

print("\n" + "="*60)
print("  [OK] TODOS LOS TESTS COMPLETADOS CORRECTAMENTE")
print("="*60)
print("\n[TIP] El sistema está listo para usar!")
print("[NOTE] Ejecuta: python src/playlist_manager.py (interfaz terminal)")
print("[BOOKS] Revisa: ejemplos_playlist.py (ejemplos de código)")
print("[BOOK] Lee: PLAYLIST_README.md (documentación completa)\n")
